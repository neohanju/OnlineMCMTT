...
/************************************************************************
 Method Name: VisualizeLidarResult
 Description:
	-
 Input Arguments:
	-
...
void CSCMTTracker::VisualizeLidarResult(
	cv::Mat _frameImage,
	hj::CTrackLidarResult _lidarResult)
{
	if (!bVisualizeResult_) { return; }

	cv::Mat resizedFrameImage;
	double rescale = IMG_WIDTH / (double)_frameImage.cols;
	double imgHeight = (double)_frameImage.rows * rescale;
	cv::resize(_frameImage, resizedFrameImage, cv::Size((int)IMG_WIDTH, (int)imgHeight));

	/* boxes */
	std::vector<hj::CObjectLidarInfo> vecObjects = _lidarResult.objectLidarInfos;
	for (size_t trajectoryIdx = 0; trajectoryIdx < vecObjects.size(); trajectoryIdx++)
	{
		hj::CObjectLidarInfo *curTrajectory = &vecObjects[trajectoryIdx];

		//// find box
		//hj::Point3D topPoint3D = curTrajectory->location;
		//topPoint3D.z += 1700;

		//hj::Point2D topPoint = WorldToImage(topPoint3D, pCalibrationInfo_);
		//hj::Point2D bottomPoint = WorldToImage(curTrajectory->location, pCalibrationInfo_);
		//double boxHeight = bottomPoint.y - topPoint.y;
		//double boxWidth = 0.5 * boxHeight;
		//hj::Rect objectBox(bottomPoint.x + 0.5 * boxWidth, bottomPoint.y, boxWidth, boxHeight);
		//objectBox *= rescale;

		//// draw box with ID
		//hj::DrawBoxWithID(resizedFrameImage, objectBox, curTrajectory->id, 1, 1, &vecColors_);

		hj::Point3D headPoint(
			curTrajectory->location.x, 
			curTrajectory->location.y, 
			1500);

		std::vector<hj::Point2D> vec3DBoxPoints = hj::GetHuman3DBox(headPoint, 500, pCalibrationInfo_);
		for (int i = 0; i < vec3DBoxPoints.size(); i++)
		{
			vec3DBoxPoints[i] *= rescale;
		}

		hj::Draw3DBoxWithID(resizedFrameImage, vec3DBoxPoints, curTrajectory->id, &vecColors_);
	}

	// DEBUG
	hj::Point2D ptX_500 = WorldToImage(hj::Point3D(500.0, .0, .0), pCalibrationInfo_);  ptX_500 *= rescale;
	hj::Point2D ptY_500 = WorldToImage(hj::Point3D(.0, 500.0, .0), pCalibrationInfo_);  ptY_500 *= rescale;
	hj::Point2D ptX_N500 = WorldToImage(hj::Point3D(-500.0, .0, .0), pCalibrationInfo_); ptX_N500 *= rescale;
	hj::Point2D ptY_N500 = WorldToImage(hj::Point3D(.0, -500.0, .0), pCalibrationInfo_); ptY_N500 *= rescale;
	cv::line(resizedFrameImage, ptX_500.cv(), ptX_N500.cv(), cv::Scalar(255.0, .0, .0));
	cv::line(resizedFrameImage, ptY_500.cv(), ptY_N500.cv(), cv::Scalar(0.0, .0, 255.0));
	...
	{
		bFirstDraw_ = false;
		cv::moveWindow(strVisWindowName_, 400, 400 + 300 * nCamID_);
	}
	cv::imshow(strVisWindowName_, resizedFrameImage);
	cv::waitKey(1);
	matTrackingResult_.release();
}
